...
def interval_overlap_score(a_start, a_end, b_start, b_end) -> float:
    "
    Fractional overlap of two closed time intervals based on intersection/union.
    Returns 0.0..1.0. Inputs can be pandas Timestamps or datetime-like.
    "
    import pandas as pd
...
def compute_overlap_matrix(df):
    "
    Compute pairwise average interval overlap across strategies in a trades DataFrame.
    Expects columns: strategy, entry_time, exit_time.
    "
    import pandas as pd
    ...
    for i, si in enumerate(strategies):
        ...
        for j, sj in enumerate(strategies):
            ...
            # Simple O(n*m) comparison; fine for CI-sized test fixtures
            total = 0.0; n = 0
...
def prune_overlap_strategies(df, *, threshold: float = 0.75, score_column: str = 'pnl'):
    "
    Drop highly overlapping strategies, keeping the one with higher total score_column.
    Returns a filtered DataFrame.
    "
    import pandas as pd
    if df.empty: return df
    mat, _ = compute_overlap_matrix(df)
    # Union-Find style selection: greedily keep best by score among pairs exceeding threshold
