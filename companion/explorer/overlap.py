...
def _normalize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "side" in df.columns:
        df["side"] = (df["side"].astype(str).str.upper().map({"BUY":"LONG","LONG":"LONG","L":"LONG","SELL":"SHORT","SHORT":"SHORT","S":"SHORT"}).fillna(df["side"]))
    for t in ("entry_time","exit_time"):
        if t in df.columns: df[t] = _to_utc(df[t])
    defaults = {"run_id":"","strategy":"","symbol":"","timeframe":"","side":"","qty":0.0,"entry_time":pd.NaT,"entry_price":float("nan"),"exit_time":pd.NaT,"exit_price":float("nan"),"pnl":0.0,"trade_id":"","position_id":""}
    for k,v in defaults.items():
        if k not in df.columns: df[k] = v
    extras = [c for c in df.columns if c not in _CANON]
    df = df[_CANON + extras]
    if "entry_time" in df.columns: df = df.sort_values("entry_time", kind="stable", na_position="last")
    return df
...
def load_trade_structure(source: Union[str, os.PathLike, pd.DataFrame], *, assume_csv_has_header: bool = True) -> pd.DataFrame:
    if isinstance(source, pd.DataFrame):
        ...
        p = Path(source)
        if not p.exists(): raise FileNotFoundError(f"Trades file not found: {p}")
        kw = {}
        if not assume_csv_has_header: kw["header"] = None
        df = pd.read_csv(p, **kw)
    df = _normalize(df)
    if "ts" in df.columns and "points" not in df.columns: df["points"] = df["ts"]
    return df
...
def interval_overlap_score(a_start, a_end, b_start, b_end) -> float:
    ...
    dur_a = (a1 - a0).total_seconds(); dur_b = (b1 - b0).total_seconds()
...
def load_master(source) -> pd.DataFrame:
    if isinstance(source, pd.DataFrame): return load_trade_structure(source)
    p = Path(source)
    if not p.exists(): raise FileNotFoundError(f"Path not found: {p}")
    frames = []
    if p.is_file():
        df = load_trade_structure(p)
        if "strategy" in df.columns and (df["strategy"] == "").all(): df["strategy"] = p.stem
        frames.append(df)
    else:
        for csv in sorted(p.glob("*.csv")):
            df = load_trade_structure(csv)
            if "strategy" in df.columns and (df["strategy"] == "").all(): df["strategy"] = csv.stem
            frames.append(df)
    if not frames: return pd.DataFrame(columns=_CANON)
...
def prune_overlap_strategies(df: pd.DataFrame, *,
    ...
    # Accept alias names
    if threshold is not None: overlap_threshold = float(threshold)
    if score_column is not None: score_col = str(score_column)
    # If this is candidate rows with trades_csv → compute Jaccard overlap on points
    if "trades_csv" in df.columns and "strategy_id" in df.columns:
        ...
        for _, r in df.iterrows():
            ...
            if pd.isna(tcsv) or not tcsv:
                rows.append((sid, set(), r)); continue
            tdf = load_trade_structure(str(tcsv))
            if "points" in tdf.columns: pts = set(tdf["points"].dropna().astype(str))
            elif "entry_time" in tdf.columns: pts = set(tdf["entry_time"].dropna().astype(str))
            else: pts = set()
...
def _score(rr):
        ...
        for sid, pts, meta in rows:
            drop = False
            for p2 in kept_pts:
                if not pts and not p2: continue
                if jaccard_points(pts, p2) >= float(overlap_threshold):
                    drop = True; break
            if not drop:
                kept_rows.append(meta); kept_pts.append(pts)
...
__all__ = ['load_trade_structure','interval_overlap_score','jaccard_points','prune_overlap_strategies','load_master','prune_master_items','save_master']
    p = Path(dest)
    if p.suffix:
