...
"""
ohlc_patch.py
Safely fixes OHLC schema/data quirks using the quarantine bundle for a run.
...
def apply_patch(run_id: str, artifacts_dir: Path) -> bool:
    ...
    try:
        ...
        marker = artifacts_dir / "ohlc_patch.ok"
        marker.write_text("patched", encoding="utf-8")
        log.info("ohlc_patch applied for run_id=%s at %s", run_id, artifacts_dir)
        return True
    except Exception as e:
        log.error("ohlc_patch failed for run_id=%s: %s", run_id, e)
        return False
