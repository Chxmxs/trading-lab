# -*- coding: utf-8 -*-
# Helpers for AI loop / MLflow (PowerShell 5 module)
...
function Invoke-PyJson {
    ...
    try { $out = & python $tmp 2>$null } catch {
...
function Get-FailedRuns {
    [CmdletBinding()]
    param([int]$LookbackHours = 24, [int]$Limit = 50)
    $py = @"
import json, time
try:
    import mlflow, pandas as pd
except Exception as e:
    print(json.dumps({"error":"mlflow_import_failed","detail":str(e)})); raise SystemExit(0)
lookback_s = $LookbackHours*3600
now_ms = int(time.time()*1000)
try:
    df = mlflow.search_runs(max_results=1000)
except Exception as e:
    print(json.dumps({"error":"mlflow_search_failed","detail":str(e)})); raise SystemExit(0)
if df.empty:
...
function Apply-LatestFailedRun {
    ...
    $rid = $items[0].run_id
    if (-not $rid) { Write-Warning "No run_id available on the latest item."; return }
